...
turns controls into a GenerateConfig.

Model families: klein is the only one with references (ReferenceLatent +
IFT); zimage and illustrious are fiat-only.
"""
import random
...
from build_payload.flux_klein import WHITE_BG_PREFIX
...
FAMILIES = {
    "klein": {"label": "Flux2 Klein", "steps": flux_klein.DEFAULT_STEPS,
              "cfg": flux_klein.DEFAULT_CFG},
    "zimage": {"label": "Z-Image Turbo", "steps": zimage.DEFAULT_STEPS,
               "cfg": zimage.DEFAULT_CFG},
    "illustrious": {"label": "Illustrious (WAI v16)", "steps": illustrious.DEFAULT_STEPS,
                    "cfg": illustrious.DEFAULT_CFG},
}
...
class GenerateConfig:
    op: str = "create"                  # "create" (fiat) | "derive" (refs + IFT)
    prompt: str = ""
    negative: str = ""
    family: str = "klein"
    ref_ids: list = field(default_factory=list)   # [ref0, extras...], alive ids only
    ...
    width: int = 1024
    height: int = 1024
    steps: int = 0                      # 0 = family default
    cfg: float = 0.0
    fresh_model: bool = False

    @classmethod
    def from_controls(cls, c, op):
        """Controls (already sanitised: dead ids dropped) -> config. Create
        ...
        family = c.get("family") or "klein"
        if family not in FAMILIES or refs or op == "derive":
            family = "klein"
        return cls(op=op, prompt=c.get("prompt") or "", negative=c.get("negative") or "",
                   ...
                   lora=c.get("lora") or "", lora_strength=float(c.get("lora_strength", 1.0)),
...
def do_generate(store, cfg, progress=None, should_abort=None, embed_workflow=False):
    ...
    lora = "" if cfg.family == "illustrious" else cfg.lora   # SDXL LoRAs: later
    ...
    for k in range(n):
        ...
        recipe = {"op": cfg.op, "prompt": cfg.prompt, "negative": cfg.negative,
                  "family": cfg.family, "steps": steps, "cfg": cfg_scale,
                  "whitebg": cfg.whitebg,
                  ...
                  "lora_strength": cfg.lora_strength if lora else None,
                  "ref0": cfg.has_ref0}
